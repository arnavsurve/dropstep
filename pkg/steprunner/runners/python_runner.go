...
func (pr *PythonRunner) Validate() error {
	...
	if step.BrowserConfig.Prompt != "" {
		return fmt.Errorf("python step %q must not define 'browser.prompt'", step.ID)
	}
	if step.BrowserConfig.UploadFiles != nil {
		return fmt.Errorf("python step %q must not define 'browser.upload_files'", step.ID)
	}
	if step.BrowserConfig.TargetDownloadDir != "" {
		return fmt.Errorf("python step %q must not define 'browser.download_dir'", step.ID)
	}
	if step.BrowserConfig.OutputSchemaFile != "" {
		return fmt.Errorf("python step %q must not define 'browser.output_schema'", step.ID)
	}
	if step.Call != nil {
		return fmt.Errorf("python step %q must not define 'call'", step.ID)
	}
	if step.BrowserConfig.AllowedDomains != nil {
		return fmt.Errorf("python step %q must not define 'browser.allowed_domains'", step.ID)
	}
	if step.BrowserConfig.MaxSteps != nil {
		return fmt.Errorf("python step %q must not define 'browser.max_steps'", step.ID)
	}
