...
`
	...
	vars, err := ResolveVarfile(varfilePath)
	...
	// Test error cases
	_, err = ResolveVarfile("nonexistent_file.yml")
	assert.Error(t, err)
	...
	_, err = ResolveVarfile(invalidPath)
	assert.Error(t, err)
...
func TestResolveStringWithContext_Json(t *testing.T) {
	globals := VarContext{"simple": "value"}
	results := StepResultsContext{
		"json_step": {
			Output: map[string]interface{}{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			result, err := resolveStringWithContext(tc.input, globals, results)
			require.NoError(t, err)
...
func TestGetNestedValue(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			val, found := getNestedValue(tc.data, tc.path)
			assert.Equal(t, tc.found, found)
			if tc.found {
				assert.Equal(t, tc.expected, val)
			}
...
func TestResolveValue(t *testing.T) {
	globals := VarContext{"key": "value"}
	results := StepResultsContext{}
	
	resolver := func(s string) (string, error) {
		return resolveStringWithContext(s, globals, results)
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			result, err := resolveValue(tc.input, resolver, globals, results)
			require.NoError(t, err)
			assert.Equal(t, tc.expected, result)
		})
	}
...
func TestResolveProviderVariables(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			result, err := ResolveProviderVariables(tc.provider, globals)
...
func TestResolveStepVariables_HttpCall(t *testing.T) {
	globals := VarContext{"base_url": "https://api.example.com"}
	results := StepResultsContext{
		"auth_step": {
			Output: map[string]interface{}{
				"token": "abc123",
